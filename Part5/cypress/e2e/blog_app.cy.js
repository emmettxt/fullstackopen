describe('Blog app', function() {
  ...
  it('Login form is shown', function() {
    cy.get('#login-form').should('not.have.css', 'display', 'none').and('be.visible')
  })
  describe('Login',function(){
    ...
    it('fails with wrong credentials', function (){
      ...
      cy.get('#notification').should('have.css','color','rgb(255, 0, 0)').and('contain','There was an error logging in,')
    })
  })
  describe('When logged in', function() {
    ...
    it('A blog can be created', function() {
      ...
      cy.get('#notification').contains('a new blog "cypress title" by cypress author added')
    ...
    describe('a blog from logged in user exists', function() {
      beforeEach(function (){
        cy.createBlog({
          title: 'a new blog',author:'a new author',url:'https://webiste.com/blogurl'
        })
      })
      ...
      it('logged in user cannot delete blog not beloning to them', function(){
        ...
        cy.get('.blog:contains("a new blog")').as('blogToDelete')
        cy.get('@blogToDelete').find('button:contains("view")').click()
        cy.get('@blogToDelete').find('button:contains("delete")').should('not.be.visible')

      })
    })
    it('blogs are displayed in order of number of likes', function (){
      //add three blogs
      cy.createBlog({ title: 'blog 1 title',author: 'blog 1 author',url: 'blog 1 url' })
      cy.createBlog({ title: 'blog 2 title',author: 'blog 2 author',url: 'blog 2 url' })
      cy.createBlog({ title: 'blog 3 title',author: 'blog 3 author',url: 'blog 3 url' })

      //expand each blog
      cy.get('.blog:contains("blog 1 title")').find('button:contains("view")').click()
      cy.get('.blog:contains("blog 2 title")').find('button:contains("view")').click()
      cy.get('.blog:contains("blog 3 title")').find('button:contains("view")').click()

      //like each blog a different number of times
      cy.get('.blog:contains("blog 1 title")').find('button:contains("like")').click()
      cy.get('.blog:contains("blog 1 title")').contains('likes 1')

      //like blog 2 twice wait
      cy.get('.blog:contains("blog 2 title")').find('button:contains("like")').click()
      ...
      cy.get('.blog:contains("blog 3 title")').find('button:contains("like")').click()
